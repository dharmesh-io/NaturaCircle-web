...
const INTENT_MAP = {
  shipping: {
    ...
    response:
      "We offer shipping across India. Standard delivery takes 5-7 business days. Shipping costs are calculated at checkout. Free shipping on orders over ₹999!",
  ...
  products: {
    ...
    followUp:
      "Options: Personal Care | Kitchen & Home | View All Products | Back to Main Menu",
  },
  personalcare: {
    keywords: [
      "personal care",
    ...
    response:
      "We offer eco-friendly personal care products including bamboo toothbrushes, shampoo bars, and skincare sets. Would you like to explore them?",
  ...
  kitchenhome: {
    ...
    response:
      "Our Kitchen & Home collection includes reusable bottles, storage solutions, and eco-friendly kitchenware. What interests you?",
    followUp: "Options: View Products | Back to Main Menu",
  },
  returns: {
    keywords: [
    ...
    response:
      "Our return policy allows returns within 7 days of delivery for unused products. Please visit our dedicated page for full details.",
  ...
  initiate_return: {
    ...
    response:
      "To start a return, please contact us directly. You can reach our support team at mcad240014@gmail.com or visit our contact page.",
  ...
  human: {
    ...
    response:
      "Connecting you to our team now. Please provide your name and email so we can assist you better.",
  ...
  track_order: {
    ...
    response:
      "To track your order, please provide your Order ID. You can find it in your confirmation email.",
  ...
  promotions: {
    ...
    response:
      "Yes! We often have exciting offers on our eco-friendly range. Check out our 'Deals' page or sign up for our newsletter to stay updated on special promotions!",
    followUp:
      "Options: View Current Deals | Sign Up for Newsletter | Back to Main Menu",
  },
  newsletter: {
    keywords: [
      "newsletter",
    ...
    response:
      "Great! Drop your email below, and we'll send you updates on new products, exclusive offers, and sustainable living tips.",
...
function extractIntent(input: string): { intent: string; confidence: number } {
  ...
  for (const item of MAIN_MENU) {
    if (cleanInput === item.toLowerCase()) {
      if (item.toLowerCase().includes("shipping"))
        return { intent: "shipping", confidence: 1 };
      if (item.toLowerCase().includes("product"))
        return { intent: "products", confidence: 1 };
      if (item.toLowerCase().includes("return"))
        return { intent: "returns", confidence: 1 };
      if (item.toLowerCase().includes("speak"))
...
export const ChatbotWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      from: "bot",
      text: "Hi there! Welcome to NaturaCircle. Sustainable Living Made Simple. 🌱 How can I help you today?",
    },
  ]);
  ...
  const handleIntent = (payload: string) => {
    ...
    if (inputMode === "name") {
      ...
      setTimeout(
        () => pushBot("Great! Now, could you please share your email address?"),
        300,
      );
    ...
    if (inputMode === "email") {
      ...
      setTimeout(
        () =>
          pushBot(
            `Perfect! Our team will get back to you shortly at ${p}. For immediate assistance, you can also reach us at ${CONTACT.email} or call ${CONTACT.phone}.`,
          ),
        300,
      );
      setTimeout(
        () => pushBot("Options: Back to Main Menu | View Products"),
        400,
      );
      setInputMode(null);
      setInput("");
    ...
    if (inputMode === "orderId") {
      ...
      setTimeout(
        () =>
          pushBot(
            `You can track your order (${p}) directly on our tracking page: /track-order`,
          ),
        300,
      );
      setTimeout(
        () => pushBot("Options: Back to Main Menu | Continue Shopping"),
        400,
      );
      setInputMode(null);
      setInput("");
    ...
    if (inputMode === "message") {
      ...
      setTimeout(
        () =>
          pushBot(
            `Thank you for your message. Our team is offline right now, but we'll get back to you within 24 hours at ${tempUserData.email || "your email"}.`,
          ),
        300,
      );
      setTimeout(
    ...
    if (intent !== "unknown" && confidence > 0) {
      ...
      if (unrecognizedCount < 1) {
        ...
        setTimeout(
          () =>
            pushBot(
              "It seems like I'm having trouble understanding. Let me connect you with our team for better assistance.",
            ),
          300,
        );
        setTimeout(
          () =>
            pushBot(
              "Please provide your name and email so we can assist you better.",
            ),
          400,
        );
